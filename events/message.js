...
module.exports = async (bot, msg) => {
    ...
    let args = [].concat.apply([], msg.content.slice(PREFIX.length).trim().split('"').map(function(v, i){
        ...
        if (command){
            try {
                ...
                if(recentCommands.includes(`${msg.author.id}-${command.name}`)){
                    return msg.channel.send(command.cdMsg ? command.cdMsg : 'You are using this command too quicky!');
                }
                ...
                await command.execute(bot, msg, args.slice(1).filter(a => a != ''), () => {
                    recentCommands = recentCommands.filter(c => c != `${msg.author.id}-${command.name}`);
                });
            } catch (err) {
                console.error(err);
